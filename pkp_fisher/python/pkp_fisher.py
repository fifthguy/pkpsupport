...
parser.add_argument('-s', dest='s', type=int, default=1, help='col names from start')
# parser.add_argument('-e', dest='e', type=int, default=0, help='col names from end')
...
start_col  = args.s
# criteria_val, criteria_col_nr, end_col = args.crit, args.c-1, args.e
test_type = args.t
...
inputs = ['../inputs/' + input1 for input1 in inputs]
...
for i, filename in enumerate(inputs):
	...
	for sheet in xl.sheet_names:
		...
		keys = df.keys()[start_col::]
		# criteria_col = df.keys()[criteria_col_nr]
		rows, cols = keys, keys

		contingency_dict = scripts.contingency_values(df, keys)
...
scripts.write_control(control_path, 'end')
